...
class GPTConfig:
    ...
    # Look-around attention: number of heads that can "look around" via learned convolution
    # 0 = disabled (use Flash Attention), >0 = use manual attention with look-around on first N heads
    n_look_around_heads: int = 0
...
class GPT(nn.Module):
    ...
    def ensure_look_around_float32(self):
        """No-op for backwards compatibility. Look-around now uses bfloat16."""
        pass
    ...
    def estimate_flops(self):
        ...
        # Exclude non-matmul params: embeddings, per-layer scalars, look-around projections
        value_embeds_numel = sum(ve.weight.numel() for ve in self.value_embeds.values())
        look_around_numel = sum(
            block.attn.look_around_proj.numel()
            for block in self.transformer.h
            if block.attn.look_around_proj is not None
        )
        nparams_exclude = (
    ...
    def num_scaling_params(self):
        ...
        look_around_params = sum(
            block.attn.look_around_proj.numel()
            for block in self.transformer.h
            if block.attn.look_around_proj is not None
        )
        transformer_matrices = (
            sum(p.numel() for p in self.transformer.h.parameters()) - look_around_params
        )
        scalars = (
            self.resid_lambdas.numel() + self.x0_lambdas.numel() + look_around_params
        )
    ...
    def setup_optimizer(
        ...
        # Extract look_around_proj params separately (small scalar-like params, use AdamW)
        look_around_params = [
            block.attn.look_around_proj
            for block in self.transformer.h
            if block.attn.look_around_proj is not None
        ]
        look_around_param_set = set(look_around_params)
        matrix_params = [
            p for p in self.transformer.h.parameters() if p not in look_around_param_set
        ]
        ...
        # Look-around projection params (small, use AdamW like other scalars)
        if look_around_params:
            param_groups.append(
                dict(
                    kind="adamw",
                    params=look_around_params,
                    lr=scalar_lr,
                    betas=adam_betas,
                    eps=1e-10,
                    weight_decay=0.0,
